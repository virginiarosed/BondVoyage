...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $dbname = "bondvoyage_db"; // Your database name
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $user['password'])) {
            ...
            header("Location: bv_home.html");
            exit();
        } else {
            // Incorrect password
            $_SESSION['error_message'] = "Wrong password. Try again.";
            header("Location: bv_login.html?error=" . urlencode($_SESSION['error_message']) . "&password_incorrect=true&email=" . urlencode($email));
            exit();
        }
    } else {
        // Email not found in the database
        $_SESSION['error_message'] = "Invalid email.";
        header("Location: bv_login.html?error=" . urlencode($_SESSION['error_message']) . "&email=" . urlencode($email));
        exit();
    }
