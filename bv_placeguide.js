...
document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch('placeguide_handler.php?action=get_destinations')
    ...
    function loadDestinationData(destination) {
        fetch(`placeguide_handler.php?action=get_places&destination=${encodeURIComponent(destination)}&search=${searchInput.value}`)
            .then(response => response.json())
            .then(data => {
                if (data.success) {
    ...
    function displayDestinationData(destination, places) {
        ...
        places.forEach(place => {
            ...
            <p><strong>Activities:</strong> ${place.activitiesHtml}</p> <!-- Render the bulleted list -->
            <div class="images">
                ${place.images.map(image => `<img src="${image}" alt="Place image" class="place-image">`).join('')}
            </div>
        `;
